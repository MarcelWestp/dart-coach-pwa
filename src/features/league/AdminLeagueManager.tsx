    ...
    <div className="p-6 max-w-6xl mx-auto flex flex-col gap-8">
      ...
      <Paper className="p-6 shadow-md">
        <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
          <div>
            <Typography variant="h5" className="font-bold flex items-center gap-2" color="text.primary">
              <EmojiEventsIcon color="primary" /> Liga-Verwaltung
            </Typography>
            <Typography variant="body2" color="textSecondary">
              Erstelle und steuere Solo-Performance Ligen oder Match-Ligen für Spieler.
            </Typography>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {leagues.map((league) => (
            <Card key={league.id} variant="outlined" className="flex flex-col justify-between">
              <CardContent>
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Chip
                    label={league.type === 'performance' ? 'Performance League' : 'Match League (1v1)'}
                    color={league.type === 'performance' ? 'primary' : 'secondary'}
                    size="small"
                  />
                </div>

                <Typography variant="body2" className="text-gray-600 dark:text-gray-300 mb-3 whitespace-pre-line">
                  {league.description}
                </Typography>

                {(league.startDate || league.endDate) && (
                  <Typography variant="caption" color="textSecondary" className="block mb-2">
                    Laufzeit: {league.startDate || 'Sofort'} bis {league.endDate || 'Unbefristet'}
                  </Typography>
                )}
                ...
                <FormControlLabel
                  ...
                  label={league.isActive ? 'Liga Aktiv' : 'Liga Pausiert/Beendet'}
                />
              </CardContent>
            </Card>
          ))}
          ...
          {monthlyConfigs.map((m) => {
            ...
            return (
              <Card key={m.id} variant="outlined">
                <CardContent>
                  <Typography variant="caption" color="primary" className="font-bold block">
      ...
      <Dialog open={isLeagueModalOpen} onClose={() => setIsLeagueModalOpen(false)} maxWidth="sm" fullWidth>
        ...
        <form onSubmit={handleCreateLeague}>
          <DialogContent dividers className="flex flex-col gap-4">
            ...
            <FormControl fullWidth required>
              ...
              >
                <MenuItem value="performance">Performance League (Solo gegen eigenen Schnitt)</MenuItem>
                <MenuItem value="match">Match League (1v1 Duelle mit TTR/ELO-System)</MenuItem>
              </Select>
            </FormControl>

            {leagueType === 'performance' && (
              <FormControl fullWidth>
                <InputLabel id="league-exercises-label">Gültige Übungen für diese Liga</InputLabel>
                <Select
                  labelId="league-exercises-label"
                  multiple
                  value={selectedExerciseIds}
                  onChange={(e) => setSelectedExerciseIds(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                  input={<OutlinedInput label="Gültige Übungen für diese Liga" />}
                  renderValue={(selected) => (
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {selected.map((exId) => {
                        const ex = exercises.find((e) => e.id === exId);
                        return <Chip key={exId} label={ex ? ex.title : exId} size="small" />;
                      })}
                    </Box>
                  )}
                >
          ...
          <DialogActions className="p-4">
            <Button onClick={() => setIsLeagueModalOpen(false)}>Abbrechen</Button>
      ...
      <Dialog open={isMonthlyModalOpen} onClose={() => setIsMonthlyModalOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle className="font-bold">Übung des Monats Festlegen</DialogTitle>
        <form onSubmit={handleCreateMonthlyExercise}>
          ...
          <DialogActions className="p-4">
            <Button onClick={() => setIsMonthlyModalOpen(false)}>Abbrechen</Button>
            <Button type="submit" variant="contained" color="primary" disabled={submitting}>
